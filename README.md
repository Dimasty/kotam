...
- 🎯 Entry points to execute your program using `python -m <kotam>` or `$ kotam` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/Dimasty/kotam/branch/main/graph/badge.svg?token=kotam_token_here)](https://codecov.io/gh/Dimasty/kotam)
[![CI](https://github.com/Dimasty/kotam/actions/workflows/main.yml/badge.svg)](https://github.com/Dimasty/kotam/actions/workflows/main.yml)

Awesome kotam created by Dimasty

## Install

from source
```bash
git clone https://github.com/Dimasty/kotam kotam
cd kotam
make install
```
...
```bash
❯ kotam --help
Usage: kotam [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ kotam create-user --help
Usage: kotam create-user [OPTIONS] USERNAME PASSWORD
...
```bash
kotam create-user admin admin --superuser
```
...
Run with `kotam run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `kotam create-user`.
...
-----------------------------------------------------
kotam/__init__.py              4      0   100%
kotam/app.py                  16      1    94%
kotam/cli.py                  21      0   100%
kotam/config.py                5      0   100%
kotam/db.py                   10      0   100%
kotam/models/__init__.py       0      0   100%
kotam/models/content.py       47      1    98%
kotam/routes/__init__.py      11      0   100%
kotam/routes/content.py       52     25    52%
kotam/routes/security.py      15      1    93%
kotam/routes/user.py          52     26    50%
kotam/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from kotam.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in kotam/default.toml.

### As environment variables
```bash
export kotam_KEY=value
export kotam_KEY="@int 42"
export kotam_KEY="@jinja {{ this.db.uri }}"
export kotam_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
kotam_ENV=production kotam run
```
